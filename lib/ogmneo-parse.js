...
class OGMNeoParse {
    ...
    static parsePropertiesArray(properties, variable = 'n') {
        if (this.isValidPropertiesArray(properties)) {
            if (_.isString(properties)) {
                return (properties === 'id') ?  `${variable}.id` : `${variable}.${properties}`;
            } else if (_.isArray(properties)) {
                return properties.reduce((result, current) => {
                    let clause = result + ((result === '') ? '' : ', ');
                    return clause + ((current === 'id') ?`${variable}.id` : `${variable}.${current}`);
                }, '');
            }
        }
        return '';
    ...
    static relationNode(record) {
        if (record) {
            ...
            obj.__type = node.type;
            obj.id = node.properties.id;
            return obj;
        }
        return null;
    }
    ...
    static propertiesFromVariable(record, variable) {
        ...
        record.keys.forEach((key) => {
            ...
            if (key === `${variable}.id`) {
                obj.id = value;
            } else if (_.startsWith(key, `${variable}.`)) {
