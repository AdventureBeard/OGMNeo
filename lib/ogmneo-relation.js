...
class OGMRelation {
    ...
    */
    ...
    static relateOperation(nodeId, type, otherNodeId, properties = {}, unique = false) {
        ...
        if (isGuid(nodeId) && isGuid(otherNodeId)) {
            if (_.isString(type) && !_.isEmpty(type)) {
                let uniqueQuery = (unique) ? 'UNIQUE' : '';
                let cypher = `MATCH (n1) WHERE n1.id='${nodeId}' ` +
                    `MATCH (n2) WHERE n2.id='${otherNodeId}' ` +
    ...
    */
    static deleteRelationOperation(relationId) {
        if (isGuid(relationId)) {
            let cypher = `MATCH p=(n1)-[r]->(n2) WHERE r.id='${relationId}' DELETE r RETURN r`;
